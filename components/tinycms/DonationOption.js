...
export default function DonationOption(props) {
  ...
  const [paymentProvider, setPaymentProvider] = useState(props.paymentProvider);
  const [monkeypodId, setMonkeypodId] = useState(props.monkeypodId);
  const [stripeId, setStripeId] = useState(props.stripeId);
  ...
  const updateStripeId = (index, value) => {
    setStripeId(value);

    let donationOptions = JSON.parse(props.parsedData.donationOptions);
    donationOptions[index].stripeId = value;
  ...
  return (
    <div key={`option-${index}`}>
      ...
      {paymentProvider === 'stripe' && (
        <div tw="mt-2">
          <label htmlFor={`donationOptions-${index}-stripeId`}>
            <span tw="mt-1 font-bold">Option Stripe Price ID</span>
            <ControlledInput
              tw="w-full rounded-md border-solid border-gray-300"
              type="text"
              name={`donationOptions-${index}-stripeId`}
              value={stripeId}
              onChange={(ev) => updateStripeId(index, ev.target.value)}
            />
          </label>
        </div>
      )}
