...
const HASURA_GET_ARTICLES_BY_SECTION = `query MyQuery($category_slug: String!, $locale_code: String!) {
...
const HASURA_LIST_ARTICLES_BY_AUTHOR = `query MyQuery($author_slug: String = "", $locale_code: String = "") {
  articles(where: {article_translations: {published: {_eq: true}, locale_code: {_eq: $locale_code}}, author_articles: {author: {slug: {_eq: $author_slug}}}}) {
    id
    slug
    article_translations {
      content
...
const HASURA_GET_AUTHOR_BY_SLUG = `query MyQuery($author_slug: String!, $locale_code: String!) {
  authors(where: {published: {_eq: true}, slug: {_eq: $author_slug}, author_translations: {locale_code: {_eq: $locale_code}}}) {
    id
    name
    photoUrl
    slug
...
const HASURA_LIST_ALL_AUTHOR_PATHS = `query MyQuery {
  authors(where: {published: {_eq: true}}) {
    slug
  }
}`;

export function hasuraListAllAuthorPaths(params) {
  return fetchGraphQL({
    url: params['url'],
    orgSlug: params['orgSlug'],
    query: HASURA_LIST_ALL_AUTHOR_PATHS,
    name: 'MyQuery',
  });
}

export function hasuraListAllArticlesForAuthor(params) {
  return fetchGraphQL({
    url: params['url'],
    orgSlug: params['orgSlug'],
    query: HASURA_LIST_ARTICLES_BY_AUTHOR,
    name: 'MyQuery',
    variables: {
...
export async function hasuraListAllArticlesBySection(params) {
  return fetchGraphQL({
    url: process.env.HASURA_API_URL,
    orgSlug: process.env.ORG_SLUG,
    query: HASURA_GET_ARTICLES_BY_SECTION,
    name: 'MyQuery',
    variables: {
