...
export async function generateAllArticlePreviewPagePaths(params = {}) {
  ...
  for (let i = 0; i < data.organizations.length; i++) {
    ...
    for (let x = 0; x < org.articles.length; x++) {
      ...
      // fixes build error - we should only build preview article pages with the required data present:
      // 	/_sites/[site]/preview/[category]/[slug]: /_sites/blackbygod/preview/
      // Error occurred prerendering page "/_sites/blackbygod/preview/". Read more: https://nextjs.org/docs/messages/prerender-error
      // Error: The provided export path '/_sites/blackbygod/preview/' doesn't match the '/_sites/[site]/preview/[category]/[slug]' page.
      // Read more: https://nextjs.org/docs/messages/export-path-mismatch
      if (siteParams['category'] && siteParams['site'] && siteParams['slug']) {
        allPaths.push({
          params: siteParams,
        });
      }
...
export async function generateAllArticleArchivePages(params = {}) {
  ...
  // CHELSEA TODO: I think there's a bug here because we're not using allPaths
  let allPaths = [];
  for (let i = 0; i < data.organizations.length; i++) {
    let org = data.organizations[i];
...
const LIST_ALL_SITE_PAGES_QUERY = `query FrontendListAllPages {
  articles(where: {article_translations: {published: {_eq: true}}}) {
    slug
    category {
      slug
...
export async function listAllPagesForSite(params) {
  return fetchGraphQL({
    url: params['url'],
    site: params['site'],
    query: LIST_ALL_SITE_PAGES_QUERY,
    name: 'FrontendListAllPages',
  });
}
