...
const HASURA_GET_LAYOUT = `query FrontendGetLayout {
  categories {
    slug
    published
    category_translations {
...
const HASURA_GET_PAGE_SLUG_VERSION = `query FrontendGetPageSlugVersion($slug: String!, $locale_code: String!) {
  page_slug_versions(where: {slug: {_eq: $slug}, page: {page_translations: {published: {_eq: true}, locale_code: {_eq: $locale_code}}}}) {
    ...
    page {
      ...
      page_translations(where: {published: {_eq: true}, locale_code: {_eq: $locale_code}}, order_by: {id: desc}) {
        content
        facebook_description
        facebook_title
        first_published_at
...
export function hasuraSaveHomepageLayout(params) {
  console.log('hasuraSaveHomepageLayout params:', params);
  return fetchGraphQL({
    url: params['url'],
    site: params['site'],
