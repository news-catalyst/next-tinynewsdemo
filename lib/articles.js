...
const HASURA_PAGINATED_ARTICLES_ARCHIVE_PAGE = `query FrontendPaginatedArticlesArchivePage($locale_code: String, $limit: Int, $offset: Int) {
  articles_aggregate(where: {article_translations: {locale_code: {_eq: $locale_code}, published: {_eq: true}}}) {
    aggregate {
      count
    }
  }
  articles(where: {article_translations: {locale_code: {_eq: $locale_code}, published: {_eq: true}}}, limit: $limit, offset: $offset, order_by: {article_translations_aggregate: {min: {first_published_at: desc}}}) {
    id
    slug
