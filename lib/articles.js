...
const LIST_ALL_SITE_ARTICLES_PAGES_QUERY = `query FrontendListAllSiteArticlesAndPages {
  articles(where: {article_translations: {published: {_eq: true}}}) {
    slug
    category {
      slug
...
export async function listAllArticlesAndPagesForSite(params) {
  return fetchGraphQL({
    url: params['url'],
    site: params['site'],
    query: LIST_ALL_SITE_ARTICLES_PAGES_QUERY,
    name: 'FrontendListAllSiteArticlesAndPages',
  });
}
