...
export async function makeFeaturedArticle(url, token, newLayoutData) {
  console.log('data for layout:', newLayoutData);

  const layoutId = newLayoutData.layoutSchemaId;
  console.log('layoutId:', layoutId);

  const articlesData = newLayoutData.articles;
    ...
    data: {
      ...
      data: {
        values: [
          {
            locale: LOCALE_ID,
            value: JSON.stringify(articlesData),
          },
        ],
      },
    },
  ...
  const data = await webinyHeadlessCms.request(CREATE_LAYOUT, variables);
  console.log('data: ', data);

  return data;
}

export async function changeLayout(url, token, newLayoutData) {
  console.log('changing layout:', newLayoutData);
