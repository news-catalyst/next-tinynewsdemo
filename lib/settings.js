...
const GET_TEST_ORG = `query FrontendGetTestOrg {
  organizations(where: {subdomain: {_eq: "next-tinynewsdemo"}}) {
    id
  }
}`;
export async function getTestOrg(params) {
  return fetchGraphQL({
    url: params['url'],
    adminSecret: process.env.HASURA_ADMIN_SECRET,
    query: GET_TEST_ORG,
    name: 'FrontendGetTestOrg',
  });
}
