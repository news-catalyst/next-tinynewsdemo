...
export function hasuraGetPagePreview({
  url,
  slug,
  adminSecret,
  orgSlug,
  ...
  if (site) {
    hasuraParams.site = site;
  } else if (adminSecret && orgSlug) {
    hasuraParams.adminSecret = adminSecret;
    hasuraParams.orgSlug = orgSlug;
  } else {
    return {
      errors: [
        new Error(
          'Validation Error: Must provide either a site name or org slug + hasura secret pair for Hasura authentication'
        ),
      ],
    };
  }
  return fetchGraphQL(hasuraParams);
}
