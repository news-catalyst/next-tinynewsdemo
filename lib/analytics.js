...
const HASURA_GET_ARTICLE_SESSIONS = `query FrontendGetArticleSessions($startDate: date!, $endDate: date!) {
  ga_article_sessions(order_by: {date: asc, count: desc}, where: {date: {_gte: $startDate, _lte: $endDate}}) {
    count
    date
    category
    document_type
...
export async function hasuraGetArticleSessions(params) {
  return fetchGraphQL({
    url: params['url'],
    orgSlug: params['orgSlug'],
    query: HASURA_GET_ARTICLE_SESSIONS,
    name: 'FrontendGetArticleSessions',
    variables: { startDate: params['startDate'], endDate: params['endDate'] },
  });
}
...
export async function hasuraInsertArticleSession(params) {
  return fetchGraphQL({
    url: params['url'],
    orgSlug: params['orgSlug'],
