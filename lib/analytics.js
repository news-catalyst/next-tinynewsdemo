...
const HASURA_GET_YESTERDAYS_DATA = `query MyQuery($startDate: date!, $endDate: date!, $donorDim: String!, $subscriberDim: String!) {
  donorDimensions: ga_custom_dimensions(where: {dimension: {_eq: $donorDim}}) {
    count
    date
    label
  }
  subscriberDimensions: ga_custom_dimensions(where: {dimension: {_eq: $subscriberDim}}) {
    count
    date
    label
  }
...
const HASURA_INSERT_DONOR_READING_FREQUENCY = `mutation MyMutation($count: Int!, $label: String!, $date: date!) {
  insert_ga_donor_reading_frequency_one(object: {count: $count, label: $label, date: $date}) {
    updated_at
    id
    created_at
    count
