...
import { listAllPagesForSite } from './articles';
...
export async function revalidateEverything(params) {
  let paths = ['/'];
  const { errors, data } = await listAllPagesForSite({
    site: params['site'],
    url: params['url'],
  });
  if (errors) {
  ...
  for (const article of data.articles) {
    paths.push(`/articles/${article.category.slug}/${article.slug}`);
  }
  for (const page of data.pages) {
    if (['about', 'donate', 'thank-you'].includes(page.slug)) {
      paths.push(`/${page.slug}`);
    } else {
      paths.push(`/static/${page.slug}`);
    }
  }
  for (const author of data.authors) {
    paths.push(`/authors/${author.slug}`);
  }
...
export const renderNewsletterContent = (content, ads, isAmp, metadata) => {
  ...
  if (content && content !== null && typeof content !== 'string') {
    // console.log('typeofcontent', typeof content, Object.keys(content));
...
// Old list for Universal analytics, left as reference during transition
export const trackingIdMapping = () => {
  return {
    'harveyworld.org': 'UA-166777432-38',
    'www.fivewardsmedia.com': 'UA-166777432-45',
    'angdiaryo.org': 'UA-166777432-51',
    'www.spotlightschools.com': 'UA-166777432-37',
    'next-tinynewsdemo': 'UA-166777432-1',
    'www.atlanticcityfocus.com': 'UA-166777432-87',
    'www.fourpointspress.com': 'UA-166777432-91',
    'www.theartrebellion.net': 'UA-166777432-89',
    dnsportscast: 'UA-166777432-94',
    'www.theoptimist.mn': 'UA-166777432-92',
    thepeoplesbeat: 'UA-166777432-90',
    news: 'UA-166777432-115',
    bpcbj: 'UA-166777432-116',
    'www.futuretides.org': 'UA-166777432-119',
    'www.thelevelernews.com': 'UA-166777432-120',
  };
};

export const ga4MeasurementIds = {
  'next-tinynewsdemo': 'G-2NRQT8YFM0',
  'angdiaryo.org': 'G-MD5CNQ9T5D',
  'www.atlanticcityfocus.com': ' G-R5ZF14JW59',
  'www.fourpointspress.com': 'G-Q5XP873H54',
  'www.fivewardsmedia.com': 'G-HJYLGLCM53',
  'www.futuretides.org': 'G-GY2CMJCH7M',
  'www.thelevelernews.com': 'G-FV1VGJVTSY',
  'www.theoptimist.mn': 'G-VQ4P8BLW5H',
  'www.spotlightschools.com': 'G-GVVSYRZQYS',
  'www.theartrebellion.net': 'G-5BVB90M0V9',
  bpcbj: 'G-XMW858C2R9',
  dnsportscast: 'G-R76FEJQR8C',
  news: 'G-SJ8XNS42CB',
  thepeoplesbeat: 'G-HRMQ8HNEF2',
};

// try to avoid Hasura rate limiting when building the site in test mode
// solution slightly modified for our use case from https://github.com/vercel/next.js/discussions/18550
export async function avoidRateLimit() {
  if (yn(process.env.NEXT_PUBLIC_CYPRESS_TESTING)) {
    console.log('... sleeping for test mode...');
    await sleep();
  }
}
