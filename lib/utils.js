...
export async function revalidate(params) {
  const { baseURL, path, site, secret } = params;

  let revalidateURL;

  if (baseURL) {
    revalidateURL = new URL(
      `/api/revalidate?site=${site}&secret=${secret}&path=${path}`,
      baseURL
    ).toString();
  } else {
    revalidateURL = `/api/revalidate?site=${site}&secret=${secret}&path=${path}`;
  }
