...
export function generateMonkeypodUrl(uuid) {
  return `https://tiny-news-collective.monkeypod.io/give/support-the-oaklyn-observer?secret=${process.env.MONKEYPOD_SECRET}&option_id=${uuid}`;
}
  ...
  if (author.slug !== null && author.slug !== undefined) {
    return (
      <Link href={`/authors/${author.slug}`} key={`${author.slug}-${i}`}>
        <ArticleAuthorLink>{author.name}</ArticleAuthorLink>
      </Link>
    );
...
export const determineTextColor = (color) => {
  ...
  console.log(color, red, green, blue, brightness);

  return brightness > 180 ? 'black' : 'white';
};

export const parsePageViews = (queryResult) => {
  let pv = {};
