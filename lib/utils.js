...
const ArticleAuthorLink = tw.a`font-bold cursor-pointer`;
...
export const renderDate = (isoDate, timeZone, renderTime = true) => {
  ...
  // use Moment-with-Timezone to convert date into the preferred time zone
  let dateInTimeZone = moment(isoDate).clone().tz(timeZone);
  let momentFormattedDate = dateInTimeZone.format('MMM. D, YYYY');
  let momentFormattedTime = dateInTimeZone.format('h:mm a z');
  try {
    if (renderTime) {
      renderedDate = `${momentFormattedDate} at ${momentFormattedTime}`;
    } else {
      renderedDate = `${momentFormattedDate}`;
    }
  } catch (e) {
