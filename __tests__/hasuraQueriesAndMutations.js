...
describe('metadata', () => {
  ...
  it('updates site metadata', () => {
    ...
    metadataParams['data'] = {
      ...
      logo:
        'https://tnc-test-upload-bucket.s3.us-east-1.amazonaws.com/oaklyn/logos/The Oaklyn Observer.jpeg',
      color: 'colortwo',
      theme: 'stylefour',
      labels:
        '{"latestNews":"Latest News","search":"Search","topics":"Topics"}',
      siteUrl: 'https://tinynewsco.org/',
      aboutCTA: 'Learn more',
      aboutDek:
        'We’re journalists for Oaklyn. We amplify community voices, share information resources, and investigate systems, not just symptoms.',
      aboutHed: 'Who We Are',
      bodyFont: 'Mulish',
      shortName: 'The Oaklyn Observer',
      subscribe:
        '{"title":"Subscribe","subtitle":"Get the latest news from Oaklyn in your inbox."}',
      supportCTA: 'Donate',
      supportDek:
        'The Oaklyn Observer exists based on the support of our readers. Chip in today to help us continue serving Oaklyn with quality journalism.',
      ...
      newsletterDek:
        'Get the latest headlines from The Oaklyn Observer right in your inbox. This is updated.',
    ...
    return hasuraUpsertMetadata(metadataParams).then((response) => {
      ...
      expect(response.data.insert_site_metadatas.returning[0]).toHaveProperty(
        'published'
      );
      expect(response.data.insert_site_metadatas.returning[0].published).toBe(
        false
      );
    });
  });
...
describe('sections', () => {
  it('creates a section', () => {
    ...
    return hasuraCreateSection(sectionParams).then((response) => {
      ...
      expect(response.data).toHaveProperty(
        'insert_categories_one.category_translations'
      );
    });
  });
  ...
  it('lists all sections', () => {
    ...
    return hasuraListAllSections(sectionParams).then((response) => {
      ...
      expect(response.data.categories[0]).toHaveProperty(
        'category_translations'
      );
    });
  });
...
describe('articles', () => {
  ...
  it('creates an unpublished article in US English', async () => {
    ...
    articleParams['content'] =
      '<p>Test article copy appears in this field.</p>';
    articleParams['search_title'] = 'Test Article 1 Search Title';
    articleParams['search_description'] =
      'Test article 1 search description copy.';
    articleParams['document_id'] =
      '1cS3u5bdBP7sg29t-nBW8UgvUHDNpiZRFccZA53A04sU';
    ...
    return hasuraCreateArticle(articleParams).then((response) => {
      ...
      expect(response.data.insert_articles.returning[0].slug).toEqual(
        articleParams['slug']
      );
      expect(response.data.insert_articles.returning[0]).toHaveProperty(
  ...
  it('adds a spanish translation to an article', async () => {
    ...
    articleParams['content'] =
      '<p>Test article spanish copy appears in this field.</p>';
    articleParams['search_title'] = 'Test Article 1 Spanish Search Title';
    articleParams['search_description'] =
      'Test article 1 spanish search description copy.';
    articleParams['document_id'] =
      '1cS3u5bdBP7sg29t-nBW8UgvUHDNpiZRFccZA53A04sU';
    articleParams['created_by_email'] = 'jacqui@newscatalyst.org';
    articleParams['article_sources'] = [];
  ...
  it('updates an article to published', async () => {
    ...
    return hasuraUpdateArticle(articleParams).then((response) => {
      ...
      expect(
        response.data.insert_articles.returning[0].article_translations[0]
          .published
      ).toEqual(true);
    });
  });
  ...
  it('adds another article', async () => {
    ...
    articleParams['content'] =
      '<p>Test article 2 copy appears in this field.</p>';
    articleParams['search_title'] = 'Test Article 2 Search Title';
    articleParams['search_description'] =
      'Test article 2 search description copy.';
    articleParams['document_id'] =
      '1cS3u5bdBP7sg29t-nBW8UgvUHDNpiZRFccZA53A0123';
    articleParams['created_by_email'] = 'jacqui@newscatalyst.org';
    articleParams['article_sources'] = [];
...
describe('pages', () => {
  it('creates an unpublished page', async () => {
    ...
    return hasuraCreatePage(pageParams).then((response) => {
      ...
      expect(response.data.insert_pages.returning[0].slug).toEqual(
        pageParams['slug']
      );
    });
  });
  ...
  it('updates a page', async () => {
    ...
    return hasuraCreatePage(pageParams).then((response) => {
      ...
      expect(response.data.insert_pages.returning[0]).toHaveProperty(
        'page_translations'
      );
      expect(
        response.data.insert_pages.returning[0].page_translations[0]
      ).toHaveProperty('published');
      expect(
        response.data.insert_pages.returning[0].page_translations[0].published
      ).toEqual(true);
    });
  });
});
